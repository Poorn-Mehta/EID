...
# AWS JSON Object and Plain Message
#AWS_Msg = {}
#AWS_JSON_Obj = 0
...
AWS_Data_Type = 0
AWS_Alert_Type = 1
AWS_Data_Type_String = "DATA"
AWS_Alert_Type_String = "ALERT"
...
# Ref: https://stackoverflow.com/questions/783897/truncating-floats-in-python
...
# AWS Message
def AWS_Format_Send_Message(Msg_Type):
    
    global Humidity_Data, Temperature_Data, TH, TL, HH, HL, C_to_F, Set_to_Celsius, Set_to_Fahrenheit
    global AWS_QoS, AWS_Data_Type, AWS_Alert_Type, AWS_Data_Type_String, AWS_Alert_Type_String
    
    AWS_Msg = {}
    
    AWS_Msg['Timestamp'] = str(datetime.now().time())
    AWS_Msg['Curr_Temp'] = float(truncate(Temperature_Data, 1))
    AWS_Msg['Curr_Humi'] = float(truncate(Humidity_Data, 1))
    
    if(C_to_F == Set_to_Fahrenheit):
        AWS_Msg['Temp_Unit'] = Set_to_Fahrenheit
            
    else:
        AWS_Msg['Temp_Unit'] = Set_to_Celsius
    
    
    if(Msg_Type == AWS_Data_Type):
        
        AWS_Msg['Type'] = AWS_Data_Type_String
        
    elif(Msg_Type == AWS_Alert_Type):
        
        AWS_Msg['Type'] = AWS_Alert_Type_String
        AWS_Msg['Temp_Alert_High'] = TH
        AWS_Msg['Temp_Alert_Low'] = TL
        AWS_Msg['Humi_Alert_High'] = HH
        AWS_Msg['Humi_Alert_Low'] = HL
    ...
    if((Humidity_Data != None) and (Temperature_Data != None)):
        ...
        if((Thr_Res.TH_Flag == Alert_Asserted) or \
           ...
           (Thr_Res.HL_Flag == Alert_Asserted)):
            ...
            AWS_Format_Send_Message(AWS_Alert_Type)
            ...
            AWS_Format_Send_Message(AWS_Data_Type)
